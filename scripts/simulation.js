...
class ProofOfStakeSimulation extends Simulation{
    constructor(node_amount){
        super()
        ...
        for (let i = 0; i < node_amount; i++){
            let new_node = null;
            if (i==0){
                new_node = new ProofOfStakeNode(this,`Node ${i}`,0,0,0,true)
                this.genesisNode = new_node
            }else{
                new_node = new ProofOfStakeNode(this,`Node ${i}`,0,0,0,false)
            }
            this.nodeList.push(new_node)
        }
